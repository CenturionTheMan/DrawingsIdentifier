...
#region Base Methods
    ...
    private double PerformLearningIteration(Matrix[] dataSamples, Matrix[] expectedResults)
    {
        ...
        Parallel.For(0, dataSamples.Length, i =>{
            ...
            Parallel.For(0, layersAmount - 1, j =>
            {
        ...
        {
            changesForWeightsSum[i] = changesForWeightsSum[i].ElementwiseMultiply(1.0 / dataSamples.Length);
            changesForBiasesSum[i] = changesForBiasesSum[i].ElementwiseMultiply(1.0 / dataSamples.Length);
        });
