...
public class ConvolutionalNeuralNetwork
{
    ...
    public void Train((double[] inputs, double[] outputs)[] data, double learningRate, double epochAmount, int batchSize, double expectedMaxError)
    {
        ...
        for (int epoch = 0; epoch < epochAmount; epoch++)
        {
            ...
            while (batchBeginIndex < data.Length)
            {
                ...
                Matrix[] expectesOutputsSamples = batchSamples.Select(x => new Matrix(x.outputs)).ToArray()!;

                double batchError = PerformLearningItearion(inputSamples, expectesOutputsSamples);
    ...
    private double PerformLearningItearion(Matrix[] dataSamples, Matrix[] expectedResuls)
    {
        ...
        for (int i = 0; i < dataSamples.Length; i++)
        {
            ...
            var changes = Backpropagation(expectedResuls[i], prediction);

            errorSum += CalculateCrossEntropyCost(expectedResuls[i], prediction);
    ...
    private double CalculateCrossEntropyCost(Matrix expected, Matrix predicitions)
    {
        if(predicitions.RowsAmount != expected.RowsAmount || predicitions.ColumnsAmount != expected.ColumnsAmount)
        {
        ...
        for (int i = 0; i < predicitions.RowsAmount; i++)
        {
            for (int j = 0; j < predicitions.ColumnsAmount; j++)
            {
                sum += expected.Values[i, j] * Math.Log(predicitions.Values[i, j]);
            }
        }
