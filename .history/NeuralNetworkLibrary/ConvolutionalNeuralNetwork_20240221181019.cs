...
public class ConvolutionalNeuralNetwork
{
    ...
    public ConvolutionalNeuralNetwork(
        (int size, ActivationFunction activationFunction) inputLayer, 
        (int size, ActivationFunction activationFunction)[] hiddenLayers, 
        (int size, ActivationFunction activationFunction) outputLayer)
    {
        int[] layersSizes = new int[hiddenLayers.Length + 2];
        ActivationFunction[] activationFunctions = new ActivationFunction[hiddenLayers.Length + 2];

        layersSizes[0] = inputLayer.size;
        activationFunctions[0] = inputLayer.activationFunction;

        for (int i = 0; i < hiddenLayers.Length; i++)
        {
            layersSizes[i + 1] = hiddenLayers[i].size;
            activationFunctions[i + 1] = hiddenLayers[i].activationFunction;
        }

        layersSizes[layersSizes.Length - 1] = outputLayer.size;
        activationFunctions[activationFunctions.Length - 1] = outputLayer.activationFunction;

        Setup(layersSizes, activationFunctions);
    }
    {
        Setup();
    }

    private void Setup(int[] layersSizes, ActivationFunction[] activationFunctions)
    {
    ...
    public void Train((double[] inputs, double[] outputs)[] data, double learningRate, double epochAmount, int batchSize, double expectedMaxError)
    {
